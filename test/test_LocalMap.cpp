...
BOOST_AUTO_TEST_CASE(test_local_map_default_constuctor)
{
    ...
    BOOST_CHECK_EQUAL(map_1->getLocalFrame().matrix().isApprox(base::Transform3d::Identity().matrix()), true); 
...
BOOST_AUTO_TEST_CASE(test_local_map_constuctor)
{
    ...
    BOOST_CHECK_EQUAL(map_2->getLocalFrame().matrix().isApprox(data->offset.matrix()), true); 

    LocalMap *map_3 = new LocalMap(data);

    BOOST_CHECK_EQUAL(map_3->getId(), data->id);
    BOOST_CHECK_EQUAL(map_3->getLocalFrame().matrix().isApprox(data->offset.matrix()), true); 
    ...
    map_3->getLocalFrame() = data_new->offset;

    BOOST_CHECK_EQUAL(map_2->getId(), data_new->id);
    BOOST_CHECK_EQUAL(map_3->getLocalFrame().matrix().isApprox(data_new->offset.matrix()), true);  

    BOOST_CHECK_EQUAL(map_2->getId(), map_3->getId());
    BOOST_CHECK_EQUAL(map_2->getLocalFrame().matrix().isApprox(map_3->getLocalFrame().matrix()), true);            
}
